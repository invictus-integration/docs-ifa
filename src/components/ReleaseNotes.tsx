...
function SeriesDropdown({ id, value, options, onChange }: {
  id: string;
  value: string;
  options: Option[];
  ...
  return (
    <div ref={containerRef} style={{ position: 'relative', display: 'inline-block' }}>
      <button
        ...
        style={{
          ...triggerStyle,
          ...(focused ? triggerFocusStyle : {}),
        }}
      >
        <span>{selected.label}</span>
        <FontAwesomeIcon
          icon={faChevronDown}
          style={{ fontSize: '0.7rem', transition: 'transform 0.2s', transform: open ? 'rotate(180deg)' : 'none' }}
        />
      </button>
...
function DropdownOption({ label, active, onSelect }: { label: string; active: boolean; onSelect: () => void }) {
  ...
  return (
    <li
      ...
      style={{
        ...optionStyle,
        ...(active ? optionActiveStyle : hover ? optionHoverStyle : {}),
      }}
    >
      {label}
    </li>
...
const triggerFocusStyle: React.CSSProperties = {
  boxShadow: '0 0 0 3px var(--ifm-color-primary)',
};
