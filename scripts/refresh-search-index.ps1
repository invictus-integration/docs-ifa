...
<#
.SYNOPSIS
    Generates knowledge.json from all versioned docs, then creates/updates
    the Azure AI Search index and uploads all documents.

.DESCRIPTION
    Reads versions.json to discover every Docusaurus documentation version,
    then for each version scans versioned_docs/version-<ver>/, extracts
    frontmatter and content from each .md/.mdx file, annotates each document
...
function Invoke-GenerateKnowledge {
    ...
    $docsDir = Join-Path $Root 'versioned_docs'     "version-$Version"
    $sidebarFile = Join-Path $Root 'versioned_sidebars' "version-$Version-sidebars.json"

    Write-Host "`n📄 Generating knowledge from docs (version: $Version)..."
    ...
    Write-Host "   ✅ $($docs.Count) documents collected"
...
function Invoke-EnsureIndex {
    ...
    if ($current) {
        ...
        # Build updated schema: live fields + new fields, preserve all live settings
        $updatedSchema = @{
            name   = $current.name
        ...
        if ($current.semanticSettings) { $updatedSchema.semanticSettings = $current.semanticSettings }

        Invoke-AzureSearch -Path "/indexes/$Index" -Method 'PUT' -Body $updatedSchema | Out-Null
        Write-Host "   ✅ Updated index — added $($newFields.Count) new field(s): $($newFields.name -join ', ')."
    }
    else {
...
if (-not (Test-Path $VersionsFile)) {
    Write-Error "versions.json not found at $VersionsFile"
    exit 1
}

$versions = Get-Content $VersionsFile -Raw | ConvertFrom-Json
if (-not $versions -or $versions.Count -eq 0) {
    Write-Error "versions.json is empty or invalid"
    exit 1
}
