...
* [`/api/CompleteExecution`:](#️complete-execution) by sending a request, it flags the currently running Azure Logic App workflow as 'completed', so that the next workflow run can continue to run.
...
<h2 id="wait-for-execution"><FontAwesomeIcon icon={faHourglassHalf} /> Wait for execution</h2>
...
<h2 id="complete-execution"><FontAwesomeIcon icon={faCheckDouble} /> Complete execution</h2>
