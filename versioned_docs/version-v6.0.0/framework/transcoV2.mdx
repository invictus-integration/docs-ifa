...
# Transco V2
The Transco function is used to promote properties from the database, by using the content or context to create the SQL query which will be executed against the specified table. Transco can also perform transformations on XML content by simply specifying the XSLT file from storage (Transco supports XSLT 1.0 syntax.).
...
:::note[basic matrix promote]
The Transco also has a dedicated matrix promotion endpoint that accepts a simple list of parameters and promotes them to the `Context`. This endpoint is not discussed here. See [Transco V2 - Matrix Functionality](./TranscoV2-Matrix.md) for more details.
:::

The Transco request requires three values:
* XML or JSON content in Base64 string format
...
The Transco response has the same structure, with the transformed content:
```json
{
...
<details>
<summary><h4 style={{ margin:0 }}>How to use TranscoV2 in Logic Apps setup</h4></summary>
