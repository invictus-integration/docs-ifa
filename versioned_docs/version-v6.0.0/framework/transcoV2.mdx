...
The component inserts the SQL query result at the specified `destination` XPath or JPath. For `XPaths` ending at an attribute (ex: `/persons/person/@name`), it inserts the value in that attribute. Otherwise, it insert the value in the inner text of the XML node.
