---
sidebar_label: Transco
title: Transco
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';
import ParameterTable from "@site/src/components/ParameterTable";
import parameters from "@site/src/data/framework.v6.bicep.parameters.json";
import { Walkthrough, Collapsible } from '@site/src/components/Walkthrough';
import { NewSinceBadge } from '@site/src/components/OnlyAdminsBadge';

{/* vale Microsoft.HeadingAcronyms = NO */}
# Promote user content in Logic App workflows with <u>Transco</u>
{/* vale Microsoft.HeadingAcronyms = YES */}
:::note[motivation]
Integration scenarios often need to 'promote' or 'enrich' messages/data from either internal or external data sources. Codit is a long-term implementer of something called 'Transco', which was originally a Microsoft BizTalk component. Now, modern Azure Logic Apps integration benefits from this behavior.

Invictus provides a Transco component to promote properties from a database or from the message itself. By using the content or context, the component creates the SQL query which executes against the specified table. Transco can also perform transformations on XML content by simply specifying the XSLT file from storage (Transco supports XSLT 1.0 syntax.).
:::

![Transco example](/images/framework/transco-v2-example.png)

<Walkthrough>
<Collapsible number="SQL" title="Promote client requests with client's database">

The Transco component can execute SQL commands to promote properties from a SQL database table.
The component creates the SQL query based on a stored configuration file and executes it against the client's database.

You can promote the result of the SQL query back to the response. 

</Collapsible>
<Collapsible number="XSLT" title="Transform client requests via stored transformation">

The Transco component can execute XSLT transformations on XML content based on a stored XSLT file.
The component applies the transformation to the request content and promotes the transformed result back to the response. 

</Collapsible>
</Walkthrough>

## Available endpoints
* `/api/TranscoXML`: uses XML content and a Transco configuration file to list the instructions necessary to promote values from a SQL database, the message itself, or to transform the content via a XSLT file.

* `/api/TranscoJson`: uses JSON content and a Transco configuration file to list the instructions necessary to promote values from a SQL database or the message itself. The component can't perform transformations on JSON content.

* `/api/MatrixBasicPromote`: accepts a list of parameters and promotes them to the `Context` in the response.

<Tabs groupId="transco-endpoints">
<TabItem value="json-xml" label="XML/JSON">

The Transco request requires following values:
* XML/JSON content in Base64 format;
* Context as key-value pair list;
* Name of Transco config file in an Azure Blob Storage container.

<details>
<summary>**Full request body JSON example**</summary>

```json
// POST /api/TranscoXML
// POST /api/TranscoJson
{
  "Content": "PD94bWwgdmVyc2lvbj0iMS4wIiBlbmNvZGluZz0idXRmLTE2Ij8+PHJvb3Q+PG5hbWUgc3RhdHVzPSJNZW1iZXIiPkpvaG4gRG9lPC9uYW1lPjwvcm9vdD4=",
  "Context": {
    "CustomerActive": "true" 
  }, 
  "TranscoConfig": "docs_config.json"
}
```
</details>

<details>
<summary>**Full response body JSON example**</summary>

```json
// 200 OK <- /api/TranscoXML
// 200 OK <- /api/TranscoJson
{
  "Content": "PD94bWwgdmVyc2lvbj0iMS4wIiBlbmNvZGluZz0idXRmLTE2Ij8+PE1lbWJlcj5Kb2huIERvZTwvTWVtYmVyPg==",
  "Context": {
    "CustomerActive": "true"
  },
  "TranscoConfig": "docs_config.json"
}
```
</details>

## Transco config file
The component requires a JSON Transco configuration file to describe the instructions for promotion. Execution of instructions happens in the order in which they appear.

<Tabs groupId="Transco-instructions">
<TabItem value="sql-command" label="SQL Commands" default>

The Transco component can execute SQL commands to promote properties from a SQL database table.

<Walkthrough>
<Collapsible title="Set the scope of the command instruction">
By default, the Transco component applies the command instruction to the entire `Content`.
You can specify a scope for the instruction by providing a `scopePath` in the configuration file.
This path is an XPath or JPath expression that points to a specific part of the content. The command applies then only to that part of the content.

```json
"scopePath": [XPath/JPath of content scope],
"namespaces": [
  {
    "namespace": [XML Namespace],
    "prefix": [XML Namespace prefix]
  }
]
// ...
```
</Collapsible>

<Collapsible title="Set the connection to the SQL database">
Provide the database connection via a raw connection string or the name of an Azure Key Vault secret holding the connection string.

```json
"command": {
   "databaseConnectionString":[Raw connection string to DB],
   "databaseKeyVaultName": [Name of DB connection string secret in Key Vault],
}
// ...
```
</Collapsible>

<Collapsible title="Set the SQL query and parameters">

You can populate the parameters of the SQL query from these sources:
 - `path` (= XML/JSON request content via XPath/JPath)
 - `context`(= Request `Context` key-value collection)
 - `value` (= Fixed value defined in the config file itself)
<br/>

```json
"command": {
  "commandValue": [SQL query to execute],
  "parameters": [
    {
      "paramName": [Name of parameter in SQL query],
      "value": [XPath/JPath if "valueType=path", Context key if "valueType=context", or any string value if "valueType=fixedValue"],
      "type": [SQL DB type. See: https://learn.microsoft.com/en-us/dotnet/api/system.data.sqldbtype],
      "valueType": ["path" or "fixedValue" or "context"]
    },
    {
      "paramName": [Name of parameter in SQL query],
      "value": [XPath/JPath if "valueType=path", Context key if "valueType=context", or any string value if "valueType=fixedValue"],
      "type": [SQL DB type. See: https://learn.microsoft.com/en-us/dotnet/api/system.data.sqldbtype],
      "valueType": ["path" or "fixedValue" or "context"]
    }
  ]
  // ...
}
```

:::tip[default parameter fallback values <NewSinceBadge version="v6.4" />]
You can specify a `defaultValue` for each parameter to fall back on in case the extracted value is `null`.
:::

Denote parameters in the SQL query with an `@` symbol. You may give parameters a name or indexed with a number.
```sql
SELECT CustomerStatus FROM dbo.Customers WHERE CustomerName = @Name AND Active = @Active*
-- or
SELECT CustomerStatus FROM dbo.Customers WHERE CustomerName = @1 AND Active = @2*
```

:::tip[control SQL query result]
With the `isMandatory` option, you control whether a `null` result of the SQL query should throw an error.
You can also specify a `defaultValue` to handle those cases.

When the query returns more than one column, you use the `columnName` of the value to be inserted in the content.

```json
"command": {
  // ...
  "isMandatory": [If true, will throw error when result is null],
  "defaultValue": [Default value of result],
  "columnName": [Obtain value from specified column if query returns more than one field. If empty, use value from first column],
}
```
:::

</Collapsible>

<Collapsible title="Set the destination of the SQL query result">

The `destination` is an XPath/JPath expression that navigates within the `Content` to insert the result of the SQL query.
For a XPath ending at an attribute (ex: `/persons/person/@name`), the component inserts the value into that attribute.
Otherwise, the component inserts the value in the inner text of the XML node.

You can also promote the SQL query result to the `Context` instead of the `Content`. In that case, set `promoteToContext=true`.

```json
"command": {
  // ...
  "destination": [XPath/JPath of the results destination, or Context key if "promoteToContext=true"],
  "promoteToContext": [If true, the component saves the SQL query result at the Context with "destination" as key]
}
```

</Collapsible>
</Walkthrough>

</TabItem>
<TabItem value="xml-transformation" label="XML Transformation">

The Transco component can execute XSLT transformations on XML content. You must specify the name of the XSLT file in the configuration file.

Specify required assemblies and dependencies used by the transformation, while the respective DLL files are to be in the storage account.

This instruction is only available when calling the `/TranscoXML` endpoint.
```json
{
  "xsltTransform": [Name of XSLT file],
  "extensions": [
    {
      "namespace": [Assembly namespace],
      "assemblyName": [Assembly name],
      "className": [Assembly class name],
      "dependencies": [
        "[DLL dependency file name]",
        "[DLL dependency file name]"
      ]
    }
  ]
}
```

</TabItem>
</Tabs>

<details>
<summary>**Full Transco config file specification**</summary>

```json
{
  "instructions": [
    {
      "scopePath": [XPath/JPath of content scope],
      "namespaces": [
        {
          "namespace": [XML Namespace],
          "prefix": [XML Namespace prefix]
        }
      ],
      "command": {
        "databaseConnectionString":[Raw connection string to DB],
        "databaseKeyVaultName": [Name of DB connection string secret in Key Vault],
        "commandValue": [SQL query to be executed],
        "isMandatory": [If true, will throw error when result is null],
        "columnName": [Obtain value from specified column if query returns more than one field. If empty, use value from first column],
        "defaultValue": [Default value of result],
        "parameters": [
          {
            "paramName": [Name of param in query],
            "value": [Type dependent. XPath or JPath if valueType = "path"],
            "type": [SQL DB type. See: https://learn.microsoft.com/en-us/dotnet/api/system.data.sqldbtype],
            "valueType": ["path" or "fixedValue" or "context"]
          },
          {
            "paramName": [Name of param in query],
            "value": [Type dependent. Any string value if valueType = "fixedValue"],
            "type": [SQL DB type. See: https://learn.microsoft.com/en-us/dotnet/api/system.data.sqldbtype],
            "valueType": ["path" or "fixedValue" or "context"]
          }
        ],
        "destination": [XPath/JPath of the results destination, or Context key if "promoteToContext=true"],
        "promoteToContext": [If true, the component saves the SQL query result to Context with "destination" as key],
        "cache": {
          "useCaching": [If true, caches the SQL query result],
          "cachingTimeout": [Cache timeout time span]
        }
      }
    },
    {
      "xsltTransform": [Name of XSLT file],
      "extensions": [
        {
          "namespace": [Assembly namespace],
           "assemblyName": [Assembly name],
           "className": [Assembly class name],
           "dependencies": [
             "[DLL dependency file name]",
             "[DLL dependency file name]"
           ]
        }
      ]
    }
  ],
  "options": {
    "configCache": {
      "useCaching": [If true, config file is cached],
      "cachingTimeout": [Cache timeout time span]
    },
    "indentResult": [If true, Transco results will be formatted and indented]
  }
 }
```
</details>

:::info[storage account]
Save all required files in the Azure Storage Account and in a Blob Storage container with the name: `Transcov2configstore` (Invictus automatically creates this container):
* `/Configs` folder with Transco config files
* `/XSLTs` folder with transformation files
* `/Assemblies` folder with assembly and dependency DLL files
:::

</TabItem>
<TabItem value="matrix-basic" label="Matrix basic promote">

The basic promote is a way of promoting values to the context. This version accept a list of parameters and promotes them to the Context. <u>The component only transfers found values to the response.</u>

:::tip
Transco XML/JSON operations also support promotion of values to the context.
* Set the option `promoteToContext=true` in the Transco configuration file.
* Set the option `destination` to the key of the desired destination in the context. 
The SQL operation will now save the result to the context.
:::

| Supported request parameter | type                                                            |
| --------------------------- | --------------------------------------------------------------- |
| `KeyValueCollection`        | Key-Value collection <br/> (each pair is individually promoted) |
| `Flow`                      | `string`                                                        |
| `Domain`                    | `string`                                                        |
| `Service`                   | `string`                                                        |
| `Action`                    | `string`                                                        |
| `Version`                   | `string`                                                        |
| `Sender`                    | `string`                                                        |
| `ApplicationName`           | `string`                                                        |
| `Milestone`                 | `string`                                                        |
| `ConversationId`            | `string`                                                        |
| `CorrelationId`             | `string`                                                        |
| `BatchId`                   | `string`                                                        |
| `Data1`                     | `string`                                                        |
| `Data2`                     | `string`                                                        |
| `Data3`                     | `string`                                                        |

<details>
<summary>**Full request body JSON example**</summary>

```json
// POST /api/api/MatrixBasicPromote
{
 "Content":  "ew0KICAiQ291bnRyeUNvZGUiOiAiQkUiLA0KICAiTW9uZXkiOiAgeyAiQW1vdW50IjogIDUwLCAiQ3VycmVuY3kiOiAgIkdCUCIgIH0NCn0NCg==",
 "Context":  {
 "x-conversationId":  "29500405-d7cf-4877-a72b-a3288cff9dc0",
 "x-correlationId":  "fc13d345-ebd7-44f2-89a9-4371258c0a08",
 "x-batchId":  "975f7ea4-6247-431b-afb6-6d27fb47516f",
 "x-applicationName":  "InvoiceApp",
 "filter":  "endtoendintegrationtests"	    
 },   
 "KeyValueCollection":  {
  "w":  3,
  "l":  10.2,
  "h":  1,
  "t":  200,
 },
 "Flow":  "fl1",
 "Domain":  "do",
 "Service":  "sr1",
 "Action":  "Ac",
 "Version":  "Vs",
 "Sender":  "Snd",
 "ApplicationName":  "Snd",
 "Milestone":  "2018",
 "ConversationId":  "29500405-d7cf-4877-a72b-a3288cff9dc0",   
 "CorrelationId":  "29500405-d7cf-4877-a72b-a3288cff9dc0", 
 "BatchId":  "29500405-d7cf-4877-a72b-a3288cff9dc0",  
 "Data1":  "d1", 
 "Data2":  "d2", 
 "Data3":  "d3"
}
```
</details>

<details>
<summary>**Full response body JSON example**</summary>

```json
// 200 OK /api/api/MatrixBasicPromote
{
    "content": "ew0KICAiQ291bnRyeUNvZGUiOiAiQkUiLA0KICAiTW9uZXkiOiAgeyAiQW1vdW50IjogIDUwLCAiQ3VycmVuY3kiOiAgIkdCUCIgIH0NCn0NCg==",
    "context": {
        "x-conversationId": "29500405-d7cf-4877-a72b-a3288cff9dc0",
        "x-correlationId": "fc13d345-ebd7-44f2-89a9-4371258c0a08",
        "x-batchId": "975f7ea4-6247-431b-afb6-6d27fb47516f",
        "x-applicationName": "InvoiceApp",
        "filter": "endtoendintegrationtests",
        "Flow": "fl1",
        "Domain": "do",	    
        "Service": "sr1",
        "Action": "Ac",
        "Version": "Vs",
        "ApplicationName": "Snd",
        "Milestone": "2018",
        "Data1": "d1",
        "Data2": "d2",
        "Data3": "d3",
        "Sender": "Snd",
        "w": 3,
        "l": 10.2,
        "h": 1,
        "t": 200
    },
    "conversationId": null,
    "correlationId": null,
    "batchId": "29500405-d7cf-4877-a72b-a3288cff9dc0"
}
```
</details>

</TabItem>
</Tabs>

## Related Bicep template parameters
<ParameterTable parameters={parameters} fixedTags={[`comp:transco`]} />