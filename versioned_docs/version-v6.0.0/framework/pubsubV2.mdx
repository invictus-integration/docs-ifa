...
<p id="pubsub-usage-desc">
  The <strong>Publisher</strong> Logic App receives a message and calls the PubSub component's [Publish](#publish-single-message) HTTP endpoint. Azure Blob Storage acts as a location to offload large message bodies. The <strong>Subscriber</strong> Logic App calls the [Subscribe](#subscribe-for-messages) HTTP endpoint to receive the messages. It calls the [Acknowledge](#acknowledge-message) HTTP endpoint to complete the message processing. The PubSub component uses Azure Service Bus as its messaging layer. 
</p>
