...
:::note[motivation]
There is no built-in Logic App action available to run regular expression replacements without using `inline code`. The Invictus **Regex Translation** Framework component fills this missing link. It provides a HTTP-endpoint to run replacements with stored translations in Azure Table Storage.
:::

## Component usage
The component has a single endpoint available to run translations. Passing an input content and the name of a stored translation, the component responds with the replaced content. The component looks up translations in the `RegexTranslator` table in Azure Table Storage.
...
<ApiPlayground
  ...
  responses={[
    ...
    },
    {
      status: 404,
      label: "Not Found",
      description: "The `RegexTranslator` table has no stored translations.",
...
Store regular expression translations in the `RegexTranslator` Azure Table Storage table (bundled with Invictus). Each row should have these properties:
...
| --------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `MatchPattern`              | The regular expression to match the incoming input `Content`. It uses [named groups] to cut certain information from the content. |
...
* **Named groups only**: the component matches with regex's `ExplicitCapture` option enabled. It doesn't capture plain `(...)` groups in `MatchPattern`. Only `(?<group-name>...)` named groups produce a usable `{group-name}` placeholder in `OutputPattern`.
