...
:::note
The ExceptionHandler function makes use of the [RegexTranslator](https://github.com/invictus-integration/docs-ifa/blob/master/framework/components/regextranslation.md) component to translate the original error from the Logic App.
:::
...
## How to use in your Azure Logic Apps?
{/* vale Microsoft.HeadingPunctuation = YES */}

Use the Exception Handler as a common exception handler Logic App that calls the Exception Handler component and return the response. 
All the other Logic Apps can call this common Logic App. This centralizes your exception handling.
...
Take a look at the code for the `Scope Exception Handling` below. Pay attention to the `Terminate` action as this uses the information from the Exception Handler component and shows it in the Dashboard.
