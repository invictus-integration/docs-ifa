...
import {faTrashCan,faPen,faCirclePlus,faFlag} from '@fortawesome/free-solid-svg-icons'
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome'

export function RemoveIcon() {
    return <FontAwesomeIcon icon={faTrashCan} color="#f43f5e" aria-hidden="true" />
}

export function EditIcon() {
    return <FontAwesomeIcon icon={faPen} color="#00a2ff" aria-hidden="true" />
}

export function AddIcon() {
    return <FontAwesomeIcon icon={faCirclePlus} color="#059669" aria-hidden="true" />
}
...
> <FontAwesomeIcon icon={faFlag} /> *The `/api/Subscribe` endpoint also needs to use a `POST` instead of a `GET` HTTP method.*
