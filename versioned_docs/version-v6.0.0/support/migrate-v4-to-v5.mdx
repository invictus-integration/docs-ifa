...
export function RemoveIcon() {
    return <FontAwesomeIcon icon={faTrashCan} color="#f43f5e" />
}

export function EditIcon() {
    return <FontAwesomeIcon icon={faPen} color="#00a2ff" />
}

export function AddIcon() {
    return <FontAwesomeIcon icon={faCirclePlus} color="#059669" />
}
...
The new Transco component does not fall back on an empty `string` when the extracted value for the parameter is `null`.
You have to explicitly set a `defaultValue` with an empty `string` to maintain the same behavior as before. <NewSinceBadge version="v6.4" />
:::
</details>
...
The new Transco now includes the basic Matrix capabilities, only authentication and endpoints requires changing.
