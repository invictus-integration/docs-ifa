...
import {faDownload,faShield,faShuffle} from '@fortawesome/free-solid-svg-icons';
...
* [Migrate PubSub](./migrate-v4-to-v5.mdx#migrate-pubsub)
* [Migrate Transco](./migrate-v4-to-v5.mdx#migrate-transco)
* [Migrate Matrix to Transco](./migrate-v4-to-v5.mdx#migrate-transco)
:::
...
[<button className="button button--medium button--primary"><FontAwesomeIcon icon={faDownload} /> Download latest version of 'Invictus-GetSources.ps1'</button>](../scripts/Invictus-GetSources.ps1)
...
## After Deployment
* [<FontAwesomeIcon icon={faShield} /> Give Invictus access to the client's Azure Logic Apps](../dashboard/installation/03_give_la_access.md)
* [<FontAwesomeIcon icon={faShuffle} /> Update redirect URLs of the app registration](https://learn.microsoft.com/en-us/entra/identity-platform/how-to-add-redirect-uri). Signing in via Microsoft Entra ID requires the new Dashboard Gateway URL.
...
* `invictus-{prefix}-flowstatistics`
...
:::warning[Remove lingering role assignments]
Removing the resources above (and the v6 installation itself) leaves lingering `Unknown` role assignments behind. If left in place, these can block later deployments with a *"role assignment already exists"* error.
:::
