...
<Walkthrough>
  ...
  <Task title="Add Azure Logic App workflow results">
    > 🛡️ You need to set some role assignments for this feature to function properly. Please see [Access Control Rights](../../installation/03_give_la_access.md) for more info.
...
If we had to take the above image as an example. If the user resubmit this flow via the Dashboard, "LogicAppChain-A" resubmits on Azure. The below image is the outcome after "LogicAppChain-A" is re-executed through the resubmit.
