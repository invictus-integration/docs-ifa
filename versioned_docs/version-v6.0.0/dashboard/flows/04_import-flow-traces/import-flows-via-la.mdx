...
<Walkthrough>
  ...
  <Task title="Link child workflow to parent Azure Logic App workflow">
    ...
    In the example above, the **x-iv-parent-workflow-run-id** was set in **LogicAppChain-B** linking it with **LogicAppChain-A**. **LogicAppChain-C** wasn't linked to **B** or **A** thus isn't considered part of the chain.
