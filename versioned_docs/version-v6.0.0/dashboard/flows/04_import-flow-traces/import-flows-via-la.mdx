...
import {faLink, faSquareCheck, faShield, faAt} from '@fortawesome/free-solid-svg-icons';
import {faSquare, faCirclePlay} from '@fortawesome/free-regular-svg-icons';
    ...
    * `EventHubsName`:
    ...
    <Tabs groupId="la">
      <TabItem value="consumption" label="Consumption">
        ...
        ![Diagnostic settings for Consumption](/images/dashboard/flowtrace/diagnostic-settings-la-consumption.png)
      ...
      ![Diagnostic settings for Standard](/images/dashboard/flowtrace/diagnostic-settings-la-standard.png)
...
When a user sets a message to ignore, its status changes and shows as follows:
...
Navigate to the *Advanced Settings* section of the flow page. To enable custom resume and resubmit for a flow, toggle the switches of **Custom resubmit URL** and **Custom resume URL** respectively. Enter a HTTPS URL in one of the fields to set a resubmit/resume URL. The <FontAwesomeIcon icon={faCirclePlay} /> button tests whether the Dashboard can reach the URL.

A message pops up to show if the test was successful.
...
Once configured, navigate to the [search flow traces](../02_search.mdx) page by selecting the flow in the sidebar you just adapted for custom resume/resubmit. You can only resume/resubmit flows based on available flow traces. 

![Clicking resubmit on a flow run](/images/v2_flowpage_clicking_resubmit.png)

The Dashboard sends a HTTP `POST` request to the configured custom resubmit URL, with a body containing information about the workflow run.
