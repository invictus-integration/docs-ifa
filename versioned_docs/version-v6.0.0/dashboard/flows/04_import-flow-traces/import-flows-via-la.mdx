...
<Walkthrough>
    ...
    * `EventHubsName`:
    ...
    <Tabs groupId="la">
      <TabItem value="consumption" label="Consumption">
        :::warning
        Make sure that the `Workflow runtime` diagnostic events is **✅ checked**, but the `AllMetrics` is **❌ unchecked**. Otherwise the app sends far too many events to the Event Hubs.
        :::
      ...
      :::warning
      Make sure that the `Function Application Logs` diagnostic events is **✅ checked**, but the `Workflow Runtime Logs` and `AllMetrics` is **❌ unchecked**. Otherwise the app sends far too many events to the Event Hubs with additional cost.
      :::

      ![Diagnostic settings for Standard](/images/dashboard/flowtrace/diagnostic-settings-la-standard.png)
    
      > 🔗 See [Microsoft's documentation](https://learn.microsoft.com/en-us/azure/logic-apps/monitor-workflows-collect-diagnostic-data?tabs=standard) on how this can be configured manually.
  ...
  <Collapsible title="Map Dashboard flows to Azure Logic App workflow runs">
  ...
  <Collapsible title="Add Azure Logic App workflow results">
    > 🛡️ You need to set some role assignments for this feature to function. Please see [Access Control Rights](../../installation/03_give_la_access.md) for more info.
...
Navigate to the *Advanced Settings* section of the flow page. To enable custom resume and resubmit for a flow, toggle the switches of **Custom resubmit URL** and **Custom resume URL** respectively. Enter a HTTPS URL in one of the fields to set a resubmit/resume URL. The `(▷)` button tests whether the Dashboard can reach the URL.
