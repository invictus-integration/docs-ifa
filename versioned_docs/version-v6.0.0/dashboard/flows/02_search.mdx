...
The Dashboard allows business users to search for flow traces for one or many flows during defect localization or verification of client integrations. The `[Select Flows]` dropdown provides you to select one or more flows for which the search query should take place.  
...
| **Status**       |    no    | Shows only the flow traces at a given state: <ul><li>`Active`: actively processing messages, ([including resubmit & resume for Logic Apps](./04_import-flow-traces/import-flows-via-la.mdx))</li><li>`Completed`: successfully finished processing.</li><li>`Error`: failure during processing ([including Logic Apps failures](./04_import-flow-traces/import-flows-via-la.mdx))</li><li>`Ignored`: [when importing via LogicApps](./04_import-flow-traces/import-flows-via-la.mdx), flows that users ignored via the Dashboard.</li></ul>  |
| **DSAV**         |    no    | Shows only flow traces with the same mapping: Domain, Service, Action, Version. (Flow trace importing treats these special business properties differently. Often loaded from 'tracked properties, when using [importing them via Logic Apps](./04_import-flow-traces/import-flows-via-la.mdx).) <br/> An extra `Contains Word` option is available to also include partial matches of these mappings.                                                                                                                                       |
| **Properties**   |    no    | Shows only flow traces that has certain custom user properties defined. You can filter for a property value using: <ul><li>`Equal`: include exact matches.</li><li>`Not Equal`: include differences</li><li>`Starts With`: include matching prefix</li><li>`Ends With`: include matching postfix</li><li>`Contains`: include partial matches.</li><li>`Regex`: include matching patterns ([regular expression language](https://learn.microsoft.com/en-us/dotnet/standard/base-types/regular-expression-language-quick-reference))</li></ul> |
