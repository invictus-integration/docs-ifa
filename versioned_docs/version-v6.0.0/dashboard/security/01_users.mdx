...
> 👤 Requires membership of a [synced Entra ID group](../installation/01_give_ad_access.mdx#sync-your-azure-active-directory-groups-to-invictus-).
...
Microsoft Entra ID users can sign in automatically into the Invictus Dashboard if that user is part of an Entra ID group that is [synced into the Dashboard](../installation/01_give_ad_access.mdx#sync-your-azure-active-directory-groups-to-invictus-).
...
Entra ID users don't have personalized roles that allow them to access certain flows in the Dashboard. The user role is determined by the Entra ID group(s) where the user is a member of. [If the group(s) has access to the flow](../installation/01_give_ad_access.mdx#sync-your-azure-active-directory-groups-to-invictus-), so will the user.
