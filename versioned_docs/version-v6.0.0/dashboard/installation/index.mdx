...
| `useResourceLocks`                 |    No    | `false`                          | The flag to indicate whether the Azure resources for the Invictus product will be deployed with [resource locks](https://learn.microsoft.com/en-us/azure/azure-resource-manager/management/lock-resources).                               |
...
| `importJobScaling`           | No       | <ul><li>`cpuResources` (`0.5`)</li><li>`memoryResources` (`1.0Gi`)</li><li>`scaleMinReplicas` (`0`)</li><li>`scaleMaxReplicas` (`20`)</li><li>`unprocessedEventThreshold` (`2000`)</li></ul> |
...
[We provide a clean-up script](../../scripts/Invictus-RemoveOldRevisions.ps1) that should be run in those cases after logging in on the correct subscription, but can be ran in Azure DevOps Pipelines as well.
:::
