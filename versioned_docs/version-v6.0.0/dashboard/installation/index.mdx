...
import { SharedNote, NewSinceBadge, DeprecatedSinceBadge, BadgedText, RowTint } from '@site/src/components/Badges';
...
import GroupCleanupScriptContent from '../../support/_groups-cleanup-script-content.mdx';
...
* **Leftover disabled groups:** if you upgrade from a version prior to v6.5, you might have leftover disabled groups in your backend storage. Remove them with the included cleanup script: <GroupCleanupScriptContent />
:::
...
- The subnets must have the following services enabled
  ...
  - `Microsoft.Storage`
- The Container App [subnet must have the delegation `Microsoft.App/environments`](https://learn.microsoft.com/en-us/azure/virtual-network/manage-subnet-delegation)
...
<details>
<summary>**Add installation script to your repository** <SharedNote/></summary>

The [`Invictus-GetSources.ps1`](../../scripts/Invictus-GetSources.ps1) script pulls the latest Invictus resources needed to deploy the Dashboard.
</details>
    ...
    - task: PowerShell@2
      ...
      inputs:
        ...
        arguments: >
          ...
          -StorageContainerName '$(Invictus.Installation.StorageAccount.Container.Name)'
...
Create a variable group (like: **{'{prefix}'}.Invictus.{'{env}'}**) for each environment. 
The deployment uses this variable group and edits/adds variables based on the Bicep deployment output.
...
| `resourcePrefix`                   | Prefix used for deployed Azure resources (ex. `invictus-{prefix}-vlt`). <Admonition type="warning" title="container app character limit"> Azure Container Apps enforce a resource name limit. Keep this prefix to **3 or less characters**.</Admonition>                                                                                                                                                                              |
...
| `useBeta` <DeprecatedSinceBadge version="v6.3" note="Use the `acrEnvironment` parameter instead." /><RowTint variant="deprecated" />                     | `$null`         | Indicates the environment of the Azure Container App registry where the deployment gets its container images.   |
{/* vale Microsoft.Terms = YES */}
...
- task: AzureCLI@2
  ...
  inputs:
    ...
    inlineScript: |
      ...
      $vnetDeployment = @{
        enabled = $true
        vnet = @{ name = 'my-vnet'; resourceGroupName = 'my-vnet-rg' }
        privateEndpoints = @{ subnetName = 'snet-pe'; resourceGroupName = 'my-rg'; names = $null }
        cae = @{ resourceGroupName = 'my-cae-infra-rg'; delegatedSubnetName = 'snet-cae' }
        privateDnsZone = @{ dnsRecords = 'create'; subscriptionId = '<sub-id>'; resourceGroupName = 'my-dns-rg' }
      }
...
The Invictus installation generates an administrator account for your first-time sign-in into the Dashboard. Use these credentials to sign in into the Dashboard for the first time. The Dashboard prompts you to reset your password after signing in.
- **Username**: `admin`
- **Password**: (`tempAdminPassword` secret value in the bundled Azure Key vault)

<br/>
:::praise[recommended for local users]
Create a new `System Admin` user with your own email address after signing in for the first time. This will help during the [Forgot Password](../security/01_users.mdx?user-type=local) procedure for local users.
:::

<hr/>

### Next steps
Follow these setup guides for customer-specific authentication and authorization:
